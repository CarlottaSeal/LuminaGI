...
**DirectLightUpdate Card Index Lookup (LUT)**
The DirectLightUpdate compute pass originally dispatched 512x512 thread groups over the full 4096x4096 atlas, with each thread performing a linear scan through all active card metadata entries to find which card owned its texel. At 372 active cards this produced ~6.2 billion structured buffer reads per dispatch, causing a 33ms single-frame spike on every light update. The fix is a 64x64 `R32_UINT` LUT (16 KB) where each texel stores the owning card index for that atlas tile (`0xFFFFFFFF` = empty). Each thread now does a single `Texture2D.Load` at `atlasCoord / 64` instead of the full scan, reducing memory accesses from 6.2B to 16.7M and eliminating the spike. This is the only precomputed LUT in the pipeline — Hammersley jitter, octahedral encoding, and BRDF SH coefficients are all evaluated analytically.
