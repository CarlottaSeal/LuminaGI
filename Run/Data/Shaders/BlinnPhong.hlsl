...
VertexOutPixelIn VertexMain( VertexInput input )
{
	...
	float4 worldTangent		= mul( c_modelToWorld, modelTangent );		// M*V order: C++ matrices are basis-major
...
//	float3 lightDir = normalize( float3( cos(0.5 * c_time), sin(0.5 * c_time), -1.0 ) );
	...
	// WARNING: HLSL float3x3 constructor is component-major (transpose of our basis-major convention above)
	...
	float3 pixelNormalWorldSpace = mul( pixelNormalTBNSpace, tbnToWorld ); // V*M order: component-major matrix
